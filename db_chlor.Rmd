...
table(new_db_chlordecone$ID,new_db_chlordecone$Date_prelevement )
...
nombre_villes_par_annee <- new_db_chlordecone %>%
  distinct(Annee,COMMU_LAB) %>%
  group_by(Annee) %>%
  summarize(Nombre_Villes_Uniques = n_distinct(COMMU_LAB))
...
liste_villes_par_annee <- combinaisons %>%
  group_by(Annee) %>%
  summarize(COMMU_LAB = paste(COMMU_LAB, collapse = ", "))
...
predictions <- data.frame(Annee = rep(unique(new_db_chlordecone$Annee), length(unique(new_db_chlordecone$COMMU_LAB))),
                          COMMU_LAB = rep(unique(new_db_chlordecone$COMMU_LAB), each = length(unique(new_db_chlordecone$Annee))))
predictions$Taux_chlordecone <- predict(modele, newdata = predictions)


ggplot(data = new_db_chlordecone, aes(x = Annee, y = Taux_chlordecone, color = COMMU_LAB)) +
  geom_point() +
...
pentes <- data.frame(COMMU_LAB = unique(new_db_chlordecone$COMMU_LAB), Pente = coefficients[2])


set.seed(123) # Pour la reproductibilité des résultats
nombre_clusters <- 3 #  ajuster le nombre de clusters 
resultats_clusters <- kmeans(pentes$Pente, centers = nombre_clusters)

# Ajouter les informations de cluster à notre dataframe de pentes
pentes$Cluster <- as.factor(resultats_clusters$cluster)

# Afficher les communes appartenant à chaque cluster
print(pentes)

# Visualisation des clusters
ggplot(pentes, aes(x = Pente, color = Cluster)) +
  geom_density() +
  labs(title = "Clusters basés sur la pente des taux de chlordécone",
       x = "Pente",
       color = "Cluster")
```
```{r}
# Calculer les dérivées des courbes de régression pour chaque commune
derivatives <- data.frame(Annee = unique(new_db_chlordecone$Annee))

# Pour chaque année, calculer la dérivée pour chaque commune
for (ville in unique(new_db_chlordecone$COMMU_LAB)) {
  # Sous-ensemble des données pour la commune spécifique
  donnees_ville <- subset(new_db_chlordecone, COMMU_LAB == ville)
  
  # Extraire les coefficients de la régression linéaire pour cette commune
  coefficients_ville <- coef(lm(Taux_chlordecone ~ Annee, data = donnees_ville))
  
  # Calculer la dérivée (coefficient de pente) pour cette commune
  derivative_ville <- coefficients_ville[2]
  
  # Ajouter la dérivée à notre dataframe
  derivatives[ville] <- derivative_ville
}


ggplot(derivatives, aes(x = Annee, y = valeur_dérivée)) +
  geom_line(aes(color = COMMU_LAB)) +
  labs(title = "Dérivées des courbes de régression pour chaque commune",
       x = "Année",
       y = "Dérivée (coefficient de pente)",
       color = "Ville")
