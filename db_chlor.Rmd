...
adresses_uniques <- distinct(new_db_chlordecone, Adresse_parcelle)
nouveau_tableau <- adresses_uniques %>%
  left_join(new_db_chlordecone, by = "Adresse_parcelle") %>%
  pivot_wider(names_from = Annee, values_fn = function(x) ifelse(is.na(x), 0, 1), values_fill = 0)
