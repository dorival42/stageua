...
ggplot(data.frame(residus), aes(x = residus)) +
  ...
  labs(title = "Histogramme des Résidus du Modèle de Régression",
       x = "Résidus",
       y = "Densité") +
  theme_minimal()
...
# Tracer l'histogramme des résidus
ggplot(data.frame(residus_transformes), aes(x = residus_transformes)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "blue", alpha = 0.7, color = "black") +
  geom_density(color = "red", size = 1) +
  labs(title = "Histogramme des résidus transformés",
       x = "Résidus",
       y = "Densité") +
  theme_minimal()
...
correlations <- cor(new_db_chlordecone[, c("Taux_chlordecone", "moyenne_pulviometrie", "mnt.exposition_mean", "mnt.ombrage_mean", "mnt.pente_mean", "mnt.tpi_mean")], use = "complete.obs")

# Affichage des résultats de corrélation
print(correlations)

# Visualisation des relations par des graphiques de dispersion
library(ggplot2)

# Fonction pour créer des graphiques de dispersion avec une ligne de régression
scatter_plot <- function(x, y, xlab, ylab) {
  ggplot(new_db_chlordecone, aes_string(x = x, y = y)) +
...
plot_list <- list(
  scatter_plot("moyenne_pulviometrie", "Taux_chlordecone", "Moyenne Pulviométrie", "Taux de Chlordécone"),
  scatter_plot("mnt.exposition_mean", "Taux_chlordecone", "Exposition Moyenne", "Taux de Chlordécone"),
  scatter_plot("mnt.ombrage_mean", "Taux_chlordecone", "Ombrage Moyen", "Taux de Chlordécone"),
  scatter_plot("mnt.pente_mean", "Taux_chlordecone", "Pente Moyenne", "Taux de Chlordécone"),
  scatter_plot("mnt.tpi_mean", "Taux_chlordecone", "TPI Moyen", "Taux de Chlordécone")
)
...
if (any(is.na(correlation_matrix))) {
    ...
    filename <- paste("LATEXDEV/correlation_", commune, ".png", sep = "")
...
# Parcourir chaque commune et créer et enregistrer le graphique de corrélation
for (commune in unique_communes) {
  commune_data <- new_db_chlordecone %>%
    filter(COMMU_LAB == commune)
  ...
  if (nrow(commune_data) > 1) {  # Vérifier qu'il y a plus d'une ligne pour calculer la corrélation
    ...
    # Enregistrer le graphique dans le dossier "images"
    filename <- paste("images/correlation_", commune, ".png", sep = "")
    dev.copy(png, filename)
    dev.off()
  }
}
...
# Assurer que les données sont bien nettoyées
data_clean <- new_db_chlordecone %>%
  mutate(across(everything(), ~ ifelse(is.na(.), mean(., na.rm = TRUE), .)))

# Sélectionner les colonnes d'intérêt pour le clustering
data_for_clustering <- data_clean %>%
  select(Taux_chlordecone, moyenne_pulviometrie, mnt.exposition_mean, mnt.ombrage_mean, mnt.pente_mean, mnt.tpi_mean)

# Normaliser les données
data_normalized <- scale(data_for_clustering)

# Appliquer le clustering K-means avec 3 clusters (par exemple)
set.seed(123)  # Pour la reproductibilité
...
fviz_cluster(kmeans_result, data = data_normalized, geom = "point",
             ...
             main = "Clustering des parcelles selon le taux de chlordécone")

# Afficher le nombre de parcelles dans chaque cluster
table(data_clean$cluster)

# Enregistrer la visualisation des clusters
ggsave("LATEXDEV/clusters_parcelles.png")

# Sauvegarder les résultats avec les clusters dans un fichier CSV
write.csv(data_clean, "images/parcelles_clusters.csv", row.names = FALSE)

# Afficher quelques lignes du résultat pour vérification
head(data_clean)
